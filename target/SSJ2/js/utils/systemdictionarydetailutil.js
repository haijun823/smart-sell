function typeid(value,row,index) {
...
$(function () {
    ...
    var method = {
        ...
        update(){
            ...
            if($selected){
                if ($selected.types){
                    $selected["types.id"] = $selected.types.id;
                }
                //将窗口绝对居中
                $addform.dialog("center").dialog("open").form("clear");
        ...
        remove(){
            ...
            if($selected){
                $.messager.confirm('确认','您确认想要删除记录吗？',function(r){
                    if (r){
                        $.get("/systemdictionarydetail/del",{id:$selected.id},function (msg) {
                            if (msg == '删除成功!!'){
                                //刷新当前页面
        ...
        //提交表单
        save(){
            var $url = "/systemdictionarydetail/add";
            if ($("#editid").val()){
                $url = "/systemdictionarydetail/upd?cmd=upd";
            }
